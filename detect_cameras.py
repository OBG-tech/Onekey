...
def generate_config_recommendation(cameras: List[Dict]):
    ...
    if not cameras:
        ...
        print("\n请检查:")
        print("  1. 相机是否已连接")
        print("  2. 系统权限设置 (macOS):")
        print("     系统偏好设置 → 安全性与隐私 → 相机")
        print("     确保允许 Terminal/iTerm 访问相机")
        print("  3. 系统权限设置 (Linux):")
        print("     用户是否在 video 组: groups $USER")
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='相机检测和配置工具 (Linux/macOS)'
    )
    parser.add_argument(
        '--test',
        type=int,
